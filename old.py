...
#             bot.reply_to(message, "Вы заблокированны в боте")
#         else:
#             return func(message)
#     return wrapper
#
...
#     ban_button = types.InlineKeyboardButton('Заблокировать пользователя', callback_data='panel_ban')
#     unban_button = types.InlineKeyboardButton('Разблокировать пользователя', callback_data='panel_unban')
#     markup.add(ban_button, unban_button)
#
#     bot.send_message(message.from_user.id, 'Добро пожаловать в админ-панель', reply_markup=markup)
#
...
#             for goal_type in contract_types_list:
#                 if goal_type in user_data[user_id].get('contract_type'):
#                     button_status = True
#                     break
#                 elif user_data[user_id].get("button_status"):
#                     button_status = True
#                 else: button_status = True
#
#             bot.edit_message_text(
...
#         button = types.InlineKeyboardButton(f'@{username}', callback_data=f'{func}_{user_id}')
#         markup.add(button)
#
#     return markup
#
...
#     user_id = call.from_user.id
#     callback = call.data
#     if callback.startswith('panel_'):
#         func = callback.replace('panel_', '', 1)
#
#         if func == 'ban':
#             markup = ban_unban_keyboard(func)
#
#             bot.send_message(user_id, 'Выберите какого пользователя Вы хотите заблокировать', reply_markup=markup)
#
#         if func == 'unban':
#             markup = ban_unban_keyboard(func)
#
#             bot.send_message(user_id, 'Выберите какого пользователя Вы хотите разблокировать', reply_markup=markup)
#
#     if callback.startswith('ban_'):
#         ban_id = int(callback.replace('ban_', '', 0))
#
#         cursor.execute('INSERT INTO banned (user_id) VALUES (?)', (ban_id,))
#         conn.commit()
#
#         username = bot.get_chat(ban_id).username
#
#         bot.send_message(user_id, f'Вы успешно заблокировали пользователя @{username}')
#
#     if callback.startswith('unban_'):
#         unban_id = int(callback[6:])
#         pass
#
#         cursor.execute("DELETE FROM banned WHERE user_id=?", (unban_id,))
#         conn.commit()
#
#         username = bot.get_chat(unban_id).username
#
#         bot.send_message(user_id, f'Вы успешно разблокировали пользователя @{username}')
#
